...
extinction_file_path = os.path.join(os.path.dirname(__file__), "..", "res", "extinction.txt")
extinction_data = np.loadtxt(extinction_file_path, skiprows=16)


def load_field(path: str, target_path: str, override_model: bool = False) -> mo.MergedField:
    """
    ...
    for star in field_stars.stars:
        ...
        coord = SkyCoord(ra=simbad_data['RA'][0], dec=simbad_data['DEC'][0], unit=(u.hourangle, u.deg), frame='icrs')
        ...
        parallax = gaia_data["parallax"].data[0] if len(gaia_data["parallax"]) > 0 else \
            float(simbad_data["PLX_VALUE"][0])
        ...
        for observation in observations:
            ...
            target_observation_path = os.path.join(target_star_path,
            ...
            for file_name, data in zip([f"{target_observation_path}.orig", f"{target_observation_path}.ndat",
        ...
        star_object = mo.MergedStar(
            ...
            abs_magnitude=float(simbad_data["FLUX_V"][0]) - 5 * np.log(distance / 10) - extinction,
            parallax=parallax,
            distance=distance
        )
    ...
    return mo.MergedField(
        path=target_path,
        stars=star_objects,
        field_number=field_stars.field_number
    )
