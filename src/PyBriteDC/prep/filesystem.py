...
def find_data(path: str) -> mf.FileSystemModel:
    ...
    fields: List[mf.FileSystemField] = [find_stars(os.path.join(path,i)) for i in os.listdir(path) if
                                        "field" in i.lower() and os.path.isdir(os.path.join(path,i))]
...
def find_stars(path: str) -> mf.FileSystemField:
    ...
    stars: List[mf.FileSystemStar] = list(itertools.chain(*[[find_observations(os.path.join(root, i)) for i in dirs if
                                                             os.path.isdir(
                                                                 os.path.join(root, i)) and regex.hd_descriptor.search(
                                                                 i) is not None] for root, dirs, found_files in
                                                            os.walk(path)]))
...
def check_if_file_valid_for_process(file_name: str, hd_number: int) -> bool:
    file_name_check = regex.file_descriptor.search(file_name) is not None and f"{hd_number}" in file_name
...
def find_observations(path: str) -> mf.FileSystemStar:
    ...
    for root, dirs, found_files in os.walk(path):
        files += [os.path.join(root, i) for i in found_files if check_if_file_valid_for_process(i, hd_number)]

    # Returns the list of satellites available for this set of observations
    available_satellites = list(set([regex.file_descriptor.search(i).group("satellite") for i in files])) # type: ignore

    observations: List[mf.FileSystemSingleObservation] = []

    def has_parts(files: List[str]) -> bool:
        return len([i for i in files if re.search(r"_part\d+", i) is not None]) == len(files)
    ...
    for satellite in available_satellites:
        ...
        for setup in list(set(available_setups)):
            orig_files = [i for i in matching_files if i.endswith(".orig2") and setup in i]
            ndat_files = [i for i in matching_files if i.endswith(".ndat") and setup in i]
            ave_files : Optional[List[str]] = [i for i in matching_files if i.endswith(".ave") and setup in i]

            if len(orig_files) != 1 and not has_parts(orig_files):
                raise ValueError(f"{path}, {satellite}, {setup} has {len(orig_files)} orig files.")

            if len(ndat_files) != 1 and not has_parts(ndat_files):
                raise ValueError(f"{path}, {satellite}, {setup} has {len(ndat_files)} ndat files.")

            if ave_files is None or len(ave_files) != 1 and not has_parts(ave_files):
                print(f"Warning: {path}, {satellite}, {setup} has {len(ave_files) if ave_files is not None else None} ave files.")
                ave_files = None
            else:
                ave_files = ave_files

            file_single_obs = mf.FileSystemSingleObservation(orig_files=orig_files,
    ...
    return mf.FileSystemStar(path=path, single_observations=observations, hd_number=hd_number)
