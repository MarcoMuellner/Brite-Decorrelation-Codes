...
from PyBriteDC.models import file as mf, observation as mo


def combine_file_data(files: List[str],
                      normalizer_fn: Optional[Callable[[np.ndarray], np.ndarray]] = None) -> Optional[np.ndarray]:
    """
...
def combine_observations(star: mf.FileSystemStar) -> List[mo.MergedSingleObservationRaw]:
    """
    ...
    for satellite in list(set([i.satellite for i in star.single_observations])):
        # get list of all ndat files and flatten it into one list
        orig_files = list(itertools.chain(
            *[[j for j in i.orig_files] for i in star.single_observations if i.satellite == satellite]))
        ndat_files = list(itertools.chain(
            *[[j for j in i.ndat_files] for i in star.single_observations if i.satellite == satellite]))
        ave_files = list(
            itertools.chain(*[[j for j in i.ave_files] for i in star.single_observations if
                              i.satellite == satellite and i.ave_files is not None]))
        ...
        if any([isinstance(i.setup, list) for i in star.single_observations if i.satellite == satellite]):
            raise ValueError("Multiple setups not yet supported in combinations.")

        data.append(mo.MergedSingleObservationRaw(
            satellite=satellite,
            setup=[i.setup for i in star.single_observations if i.satellite == satellite],
            orig_data=orig_data,
            ndat_data=ndat_data,
            ave_data=ave_data
        ))
