...
test_data_decor_path_bad = os.path.join(os.path.dirname(__file__), "testdata", "decorrelations_structure_bad")
test_data_decor_path_good = os.path.join(os.path.dirname(__file__), "testdata", "decorrelations_structure_good")
...
@pytest.mark.parametrize("file_name, hd_number, expected", [
    ...
    ("HD138690_09-Sco-I-2015_BLb_['4']_['5']_['3']_['2']_['6']_['1']_3_merged_.ndat", 138690, False)
])
...
@pytest.mark.parametrize("path, expected", [
    (os.path.join(test_data_decor_path_bad, "Field 1 - Ori I (Andrzej)", "HD31237"), 3),
    (os.path.join(test_data_decor_path_bad, "Field 1 - Ori I (Andrzej)", "HD37043"), 3),
...
def test_combine_star_data():
    star = fs.find_observations(os.path.join(test_data_with_content, "Field_1_with_content", "HD31237"))
    result = cb.combine_observations(star)
    assert len(result) == 2

    for satellite, setups in zip([ob.SatelliteEnum.BAb, ob.SatelliteEnum.UBr], [[3, 4], [7]]):
